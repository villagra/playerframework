    ...
    public sealed class CaptionBlockRegion : Control
    {
        ...
        private void ApplyRegionStyles()
        {
            if (isTemplateApplied)
            {
                ...
                CaptionsBorder.Background = GetCachedBrush(CaptionRegion.CurrentStyle.WindowBackgroundColor);
        ...
        private UIElement RenderElement(CaptionElement element)
        {
            ...
            try
            {
                ...
                {
                    var border = new Border() { Background = GetCachedBrush(element.CurrentStyle.RegionBackgroundColor) };
                    border.Child = parent;
                    result = border;
                ...
                {
                    result = parent;
                    parent.Background = GetCachedBrush(element.CurrentStyle.RegionBackgroundColor);
                }
        ...
        private double WrapElement(Panel parent, ref Panel p, double offset, string text, CaptionElement element, TextAlignment align, bool directionApplied = false)
        {
            ...
            border.Background = GetCachedBrush(style.RegionBackgroundColor);
        ...
        private TextBlock GetStyledTextblock(TimedTextStyle style, double width, double height, bool fOutline)
        {
            ...
            if(style.FontCapitals != FontCapitals.Normal)
            {
                textblock.FontFamily = style.FontFamily.WindowsFontFamily;
                Typography.SetCapitals(textblock, style.FontCapitals);
            }
            else if (style.FontFamily.Source == "_Smallcaps")
            {
